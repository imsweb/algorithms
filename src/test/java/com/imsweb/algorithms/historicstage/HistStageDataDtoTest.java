...
public class HistStageDataDtoTest {
    ...
    public void testParse() {
        ...
        range = Range.between(1, 9999);
        Assert.assertTrue(result.contains(range));
        range = Range.between(0, 75);
        Assert.assertTrue(result.contains(range));
        range = Range.between(345, 346);
        Assert.assertFalse(result.contains(range));
        range = Range.between(7, 8);
        Assert.assertFalse(result.contains(range));
        range = Range.between(8, 9);
        Assert.assertFalse(result.contains(range));
        range = Range.between(7, 9);
        Assert.assertFalse(result.contains(range));
        range = Range.between(-8000, 2000);
        Assert.assertFalse(result.contains(range));
        range = Range.between(932, 999);
        Assert.assertFalse(result.contains(range));
