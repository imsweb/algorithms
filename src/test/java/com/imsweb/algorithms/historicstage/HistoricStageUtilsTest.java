...
public class HistoricStageUtilsTest {
    ...
    public void testComputeHistoricStage() {
        ...
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2012");
        record.put(_PROP_TYPE_OF_REPORTING_SOURCE, "7");
        record.put(_PROP_PRIMARY_SITE, "808");
        record.put(_PROP_HISTOLOGY_3, "9823");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        //Test 2004+ leukemia DCO cases
        record.put(_PROP_PRIMARY_SITE, "424");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "100");
        record.put(_PROP_CS_LYMPH_NODES, "100");
        record.put(_PROP_CS_METS_AT_DX, "10");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_METS_AT_DX, "00");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        //Test 2004+ Non DCO cases
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2004");
        record.put(_PROP_TYPE_OF_REPORTING_SOURCE, "1");
        record.put(_PROP_PRIMARY_SITE, "003");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_HISTOLOGY_3, "9130");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "405");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_LYMPH_NODES, "110");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_METS_AT_DX, "00");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());

        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2013");
        record.put(_PROP_PRIMARY_SITE, "099");
        record.put(_PROP_HISTOLOGY_3, "9935");
        record.put(_PROP_CS_EXTENSION, "900");
        record.put(_PROP_CS_LYMPH_NODES, "310");
        record.put(_PROP_CS_METS_AT_DX, "40");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "800");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());

        //Test Special case for longevity consistency
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2007");
        record.put(_PROP_PRIMARY_SITE, "400");
        record.put(_PROP_CS_EXTENSION, "800");
        record.put(_PROP_CS_LYMPH_NODES, "100");
        record.put(_PROP_CS_METS_AT_DX, "99");
        record.put(_PROP_HISTOLOGY_3, "9827");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());

        //Test special case for corpus schemas per Lynn Ries/Jennifer Ruhl starting Nov11 sub
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2010");
        record.put(_PROP_PRIMARY_SITE, "541");
        record.put(_PROP_HISTOLOGY_3, "9882");
        record.put(_PROP_CS_EXTENSION, "530");
        record.put(_PROP_CS_LYMPH_NODES, "000");
        record.put(_PROP_CS_METS_AT_DX, "00");
        Assert.assertEquals(HistoricStageUtils.LOCALIZED, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_SITE_SPECIFIC_FACTOR2, "010");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());

        //test Special Breast Processing
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2009");
        record.put(_PROP_PRIMARY_SITE, "505");
        record.put(_PROP_HISTOLOGY_3, "8000");
        record.put(_PROP_CS_EXTENSION, "710");
        record.put(_PROP_CS_LYMPH_NODES, "130");
        record.put(_PROP_CS_METS_AT_DX, "00");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "070");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_BEHAVIOR_3, "2");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_BEHAVIOR_3, "3");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "050");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "710");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        ...
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2008");
        record.put(_PROP_PRIMARY_SITE, "384");
        record.put(_PROP_HISTOLOGY_3, "9995");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "400");
        record.put(_PROP_CS_LYMPH_NODES, "800");
        record.put(_PROP_CS_METS_AT_DX, "10");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "100");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_SITE_SPECIFIC_FACTOR1, "40");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_SITE_SPECIFIC_FACTOR1, "10");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_SITE_SPECIFIC_FACTOR1, "999");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_SITE_SPECIFIC_FACTOR1, "20");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "500");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_SITE_SPECIFIC_FACTOR1, "10");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "670");
        Assert.assertEquals(HistoricStageUtils.NEED_REVIEW, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "770");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        record.put(_PROP_CS_EXTENSION, "999");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());

        //Test 2004 earlier cases
        record.clear();
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_PRIMARY_SITE, "");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "1899");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "1990");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_PRIMARY_SITE, "805");
        record.put(_PROP_HISTOLOGY_3, "8733");
        Assert.assertEquals(HistoricStageUtils.NOT_APPLICABLE, computeHistoricStage(record).getResult());
        //coding system 4
        record.put(_PROP_EOD_CODING_SYSTEM, "4");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        //test DCO cases
        record.put(_PROP_TYPE_OF_REPORTING_SOURCE, "7");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_PRIMARY_SITE, "505");
        record.put(_PROP_HISTOLOGY_3, "9733");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        //test Los angeles 1988-1991 cases
        record.put(_PROP_REGISTRY_ID, "000001535");
        Assert.assertEquals(HistoricStageUtils.NOT_APPLICABLE, computeHistoricStage(record).getResult());
        //Non dco cases, coding system 4
        record.put(_PROP_REGISTRY_ID, "000001536");
        record.put(_PROP_TYPE_OF_REPORTING_SOURCE, "3");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_PRIMARY_SITE, "C005");
        record.put(_PROP_HISTOLOGY_3, "9542");
        record.put(_PROP_EOD10_LYMPH_NODES, "3");
        record.put(_PROP_EOD10_EXTENSION, "10");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
        //coding system 4, breast cases
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "1990");
        record.put(_PROP_EOD_CODING_SYSTEM, "4");
        record.put(_PROP_PRIMARY_SITE, "C505");
        record.put(_PROP_EOD10_EXTENSION, "05");
        record.put(_PROP_HISTOLOGY_3, "9127");
        record.put(_PROP_EOD10_LYMPH_NODES, "0");
        Assert.assertEquals(HistoricStageUtils.IN_SITU, computeHistoricStage(record).getResult());
        record.put(_PROP_BEHAVIOR_3, "2");
        Assert.assertEquals(HistoricStageUtils.IN_SITU, computeHistoricStage(record).getResult());
        record.put(_PROP_BEHAVIOR_3, "3");
        Assert.assertEquals(HistoricStageUtils.LOCALIZED, computeHistoricStage(record).getResult());
        ...
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "1990");
        record.put(_PROP_EOD_CODING_SYSTEM, "4");
        record.put(_PROP_HISTOLOGY_3, "8770");
        record.put(_PROP_PRIMARY_SITE, "C444");
        record.put(_PROP_EOD10_EXTENSION, "99");
        record.put(_PROP_EOD10_LYMPH_NODES, "3");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
        //coding system 3, EOD patch table
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2000");
        record.put(_PROP_EOD_CODING_SYSTEM, "3");
        record.put(_PROP_TYPE_OF_REPORTING_SOURCE, "3");
        record.put(_PROP_HISTOLOGY_3, "9805");
        record.put(_PROP_PRIMARY_SITE, "C444");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        ...
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2000");
        record.put(_PROP_EOD_CODING_SYSTEM, "3");
        record.put(_PROP_HISTOLOGY_3, "9133");
        record.put(_PROP_PRIMARY_SITE, "C035");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        record.put(_PROP_EOD4_DIGIT_EXTENSION, "0");
        record.put(_PROP_EOD4_DIGIT_NODES, "0");
        Assert.assertEquals(HistoricStageUtils.IN_SITU, computeHistoricStage(record).getResult());
        record.put(_PROP_EOD4_DIGIT_EXTENSION, "1");
        Assert.assertEquals(HistoricStageUtils.LOCALIZED, computeHistoricStage(record).getResult());
        //coding system 3, ovary
        record.put(_PROP_PRIMARY_SITE, "C569");
        record.put(_PROP_EOD4_DIGIT_SIZE, "02");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        //coding system 2, Lung cases
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2000");
        record.put(_PROP_EOD_CODING_SYSTEM, "2");
        record.put(_PROP_HISTOLOGY_3, "9133");
        record.put(_PROP_PRIMARY_SITE, "C347");
        record.put(_PROP_EOD13_DIGIT, "111-1&986-000");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
        ...
        record.put(_PROP_EOD_CODING_SYSTEM, "2");
        record.put(_PROP_HISTOLOGY_3, "8733");
        record.put(_PROP_PRIMARY_SITE, "C447");
        record.put(_PROP_EOD13_DIGIT, "111-23-8--100");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        ...
        record.put(_PROP_EOD_CODING_SYSTEM, "2");
        record.put(_PROP_HISTOLOGY_3, "8733");
        record.put(_PROP_PRIMARY_SITE, "C677");
        record.put(_PROP_EOD13_DIGIT, "111-83-821000");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
        ...
        record.put(_PROP_EOD_CODING_SYSTEM, "2");
        record.put(_PROP_HISTOLOGY_3, "8733");
        record.put(_PROP_PRIMARY_SITE, "C502");
        record.put(_PROP_EOD13_DIGIT, "1111-&-821000");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
        //coding system 2,other histologies than 8000-9589
        record.put(_PROP_HISTOLOGY_3, "9590");
        Assert.assertEquals(HistoricStageUtils.UNSTAGED, computeHistoricStage(record).getResult());
        //coding system 1, EOD 2 dig direct stage
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2000");
        record.put(_PROP_EOD_CODING_SYSTEM, "1");
        record.put(_PROP_HISTOLOGY_3, "8733");
        record.put(_PROP_PRIMARY_SITE, "C645");
        record.put(_PROP_EOD2_DIGIT, "2&");
        Assert.assertEquals(HistoricStageUtils.LOCALIZED, computeHistoricStage(record).getResult());
        //coding system 1, EOD 2 dig Ext Node stage
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2000");
        record.put(_PROP_EOD_CODING_SYSTEM, "1");
        record.put(_PROP_HISTOLOGY_3, "8700");
        record.put(_PROP_PRIMARY_SITE, "C160");
        record.put(_PROP_EOD2_DIGIT, "&2");
        Assert.assertEquals(HistoricStageUtils.DISTANT, computeHistoricStage(record).getResult());
        //coding system 0
        record.clear();
        record.put(_PROP_DATE_OF_DIAGNOSIS_YEAR, "2000");
        record.put(_PROP_EOD_CODING_SYSTEM, "0");
        record.put(_PROP_HISTOLOGY_3, "8700");
        record.put(_PROP_PRIMARY_SITE, "C560");
        record.put(_PROP_EOD2_DIGIT, "7-");
        Assert.assertEquals(HistoricStageUtils.REGIONAL, computeHistoricStage(record).getResult());
    }
    ...
    private HistoricStageResultsDto computeHistoricStage(Map<String, String> record) {
        HistoricStageInputDto input = new HistoricStageInputDto();
        input.setDateOfDiagnosisYear(record.get(_PROP_DATE_OF_DIAGNOSIS_YEAR));
        input.setPrimarysite(record.get(_PROP_PRIMARY_SITE));
        input.setHistologyIcdO3(record.get(_PROP_HISTOLOGY_3));
        input.setBehaviorIcdO3(record.get(_PROP_BEHAVIOR_3));
        input.setTypeOfReportingSource(record.get(_PROP_TYPE_OF_REPORTING_SOURCE));
        input.setRegistryId(record.get(_PROP_REGISTRY_ID));
        input.setCsExtension(record.get(_PROP_CS_EXTENSION));
        input.setCsLymphNodes(record.get(_PROP_CS_LYMPH_NODES));
        input.setCsMetsAtDx(record.get(_PROP_CS_METS_AT_DX));
        input.setCsSiteSpecificFactor1(record.get(_PROP_CS_SITE_SPECIFIC_FACTOR1));
        input.setCsSiteSpecificFactor2(record.get(_PROP_CS_SITE_SPECIFIC_FACTOR2));
        input.setEodCodingSys(record.get(_PROP_EOD_CODING_SYSTEM));
        input.setEodExtension(record.get(_PROP_EOD10_EXTENSION));
        input.setEodLymphNodeInvolv(record.get(_PROP_EOD10_LYMPH_NODES));
        input.setEodOld4DigitExtent(record.get(_PROP_EOD4_DIGIT_EXTENSION));
        input.setEodOld4DigitNodes(record.get(_PROP_EOD4_DIGIT_NODES));
        input.setEodOld4DigitSize(record.get(_PROP_EOD4_DIGIT_SIZE));
        input.setEodOld13Digit(record.get(_PROP_EOD13_DIGIT));
        input.setEodOld2Digit(record.get(_PROP_EOD2_DIGIT));
